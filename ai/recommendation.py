...
class RecommendationEngine:
    ...
    def recommend(cls, token: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The production scanner is multi-chain.  Keep recommendation
        # chain-aware, but do not reject valid Base tokens simply because
        # the older recommendation engine was Solana-only.
        ...
        if chain and chain not in supported_chains:
            ...
            risk_flags.append(
                f"UNSUPPORTED CHAIN: {chain.upper()}"
            )
        ...
        if security_score < cls.MIN_SECURITY_SCORE:
            ...
            risk_flags.append(
                f"SECURITY BELOW {cls.MIN_SECURITY_SCORE}"
            )

        # Defense-in-depth for Base/EVM tokens.  The primary security
        # engine already performs these checks; recommendation must never
        # upgrade a token that clearly failed them.
        if chain == "base":

            if cls._bool(token.get("honeypot_detected")):
            ...
            if (
                token.get("honeypot_available") is False
                and not cls._bool(
                    token.get("security_should_pass")
                )
            ):
                ...
                risk_flags.append(
                    "BASE SECURITY DATA UNAVAILABLE"
                )

            sell_tax = cls._number(
                token.get("sell_tax")
            )
