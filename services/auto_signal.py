...
AUTO SIGNAL POLICY:
    - STRONG GEM -> SEND
    - GEM SIGNAL -> SEND
    - EARLY GEM -> TELEGRAM SIGNAL (security-gated)
    - WATCH -> MONITOR ONLY
    - NO SIGNAL -> IGNORE
...
class AutoSignalEngine:
    ...
    FORMAT_VERSION = "V10-MULTICHAIN-SIGNAL-INTEGRITY"
    ...
    async def start(self):
        ...
        print(
            "🟢 Robinhood Chain scanner: ENABLED"
        )
    ...
    async def scan_once(self):
        ...
        for chain_name in self.SUPPORTED_CHAINS:
            chain_results_map[chain_name] = await self._scan_chain(
                chain_name
            )
        ...
        for chain_results in chain_results_map.values():
        ...
        for chain_name, chain_results in chain_results_map.items():
            stats = getattr(
                self.scanner,
                "last_scan_stats",
                {},
        ...
        self.last_scan_summary = {
            ...
            "base_candidates": len(chain_results_map.get("base", [])),
            "robinhood_candidates": len(chain_results_map.get("robinhood", [])),
            "arc_candidates": len(chain_results_map.get("arc", [])),
    ...
    def _signal_rejection_reason(
        ...
        final_signal = str(token.get("final_signal", "")).upper().strip()
        if final_status not in {"STRONG GEM", "GEM SIGNAL", "EARLY GEM"}:
            return "final_status"
        if final_signal not in {"🔥 STRONG GEM", "🚀 GEM SIGNAL", "👀 EARLY GEM"}:
            return "final_signal_integrity"

        # FinalSignalEngine is authoritative for STRONG GEM / GEM SIGNAL / EARLY GEM.
        # No second confirmed_gem gate is applied here.

        if token.get("false_signal_penalty", 0) >= 25:
            return "false_signal"
...
# recommendation engine marks is_recommended=False.
    ...
    def format_signal(
        ...
        chain_name = {
            ...
            "arc": "🟠 Arc",
        }.get(chain, chain.upper() or "Unknown")
        ...
        v9_confidence = cls._int(token.get("confidence_2", confidence))
        v9_gem = cls._int(token.get("gem_score_2", gem_score))
        liquidity_quality = cls._int(token.get("liquidity_quality_score", 0))
        momentum = cls._int(token.get("momentum_score", 0))
        smart_money = cls._int(token.get("smart_money_score", 0))
        holder_score = cls._int(token.get("holder_score", 0))
        creator_score = cls._int(token.get("creator_score", 0))
        volume_quality = cls._int(token.get("volume_quality_score", 0))
        early_entry = cls._int(token.get("early_entry_score", 0))
        confirmation_count = cls._int(token.get("confirmation_count", 0))
        ...
        if chain == "base":
            ...
            scan_url = f"https://robinhoodchain.blockscout.com/address/{encoded_address}" if encoded_address else ""
            scan_label = "Robinhood Explorer"
            gecko_url = ""
            gmgn_url = ""
        elif chain == "arc":
            scan_url = f"https://explorer.arc.io/address/{encoded_address}" if encoded_address else ""
            scan_label = "Arc Explorer"
            gecko_url = ""
            gmgn_url = ""
        else:
        ...
        lines.extend([
            "🛡 Security: <b>⚠️ REVIEW</b>" if security_score < 69 else "🛡 Security: <b>✅ PASS</b>",
            "━━━━━━━━━━━━━━━━━━━━",
        ])
        ...
        lines.extend([
            ...
            f"┗ GEM Score 2.0: <b>{v9_gem}/100</b>",
            f"┗ Security Score: <b>{security_score}/100</b>",
            f"┗ Final Score: <b>{final_score}/100</b>",
            f"┗ Liquidity: <b>{liquidity_quality}/100</b> • Momentum: <b>{momentum}/100</b>",
            f"┗ Volume Quality: <b>{volume_quality}/100</b> • Early Entry: <b>{early_entry}/100</b>",
            f"┗ Holders: <b>{holder_score}/100</b> • Smart Money: <b>{smart_money}/100</b>",
            f"┗ Creator: <b>{creator_score}/100</b> • Confidence: <b>{v9_confidence}%</b>",
            f"┗ Confirmation: <b>{confirmation_count}/3</b>",
        ])
